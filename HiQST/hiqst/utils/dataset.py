...
This file is adapted from Robomimic
https://github.com/ARISE-Initiative/robomimic/blob/master/robomimic/utils/dataset.py
"""
import os
import h5py
import numpy as np
...
import hiqst.utils.tensor_utils as TensorUtils
import hiqst.utils.obs_utils as ObsUtils
...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        assert self.obs_seq_length >= 1
        self.lowdim_obs_seq_length = lowdim_obs_seq_length
    ...
    def get_item(self, index):
        ...
        if self.lowdim_obs_seq_length is None:
            ...
            high_dim_keys = [key for key in self.obs_keys if not ObsUtils.key_is_obs_modality(key, "low_dim")]
            low_dim_keys = [key for key in self.obs_keys if ObsUtils.key_is_obs_modality(key, "low_dim")]
            ...
            meta["obs"].update(self.get_obs_sequence_from_demo(
                demo_id,
                index_in_demo=index_in_demo,
                keys=low_dim_keys,
                num_frames_to_stack=self.n_frame_stack,
                seq_length=self.lowdim_obs_seq_length,
                prefix="obs"
            ))
        ...
        if self.load_next_obs:
            ...
            # I'm redefining next obs to be the observation after the sequence

            next_obs_start = min(index_in_demo + self.seq_length, demo_length - 1)
            meta["next_obs"] = self.get_obs_sequence_from_demo(
                demo_id,
                index_in_demo=next_obs_start,
                keys=self.obs_keys,
                num_frames_to_stack=self.n_frame_stack - 1,
                seq_length=1,
                prefix="obs"
