...
import hiqst.utils.tensor_utils as TU
...
def obs_encoder_kwargs_from_config(obs_encoder_config):
    ...
    for obs_modality, encoder_kwargs in obs_encoder_config.items():
        ...
        for cls_name, cores in zip(("core", "obs_randomizer"), (OBS_ENCODER_CORES, OBS_RANDOMIZERS)):
            # Make sure the rehiqsted encoder for each obs_modality exists
...
def process_frame(frame, channel_dim, scale=None):
    """
...
def crop_image_from_indices(images, crop_indices, crop_height, crop_width):
    ...
    crop_ind_grid_h = torch.arange(crop_height, device=device)
    ...
    crop_ind_grid_w = torch.arange(crop_width, device=device)
